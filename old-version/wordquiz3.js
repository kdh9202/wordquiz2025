...
const randColorArray=['#cd84f1', '#ffcccc', '#ffaf40','#fffa65', '#c56cf0','#ffb8b8','#ff9f1a','#fff200','#32ff7e','#7efff5','#18dcff','#7d5fff','#3ae374','#67e6dc','#17c0eb'];
let rand=function(){
...
const openAnswerBtn=document.querySelector('.openAnswerBtn');
const userQuizInput=document.querySelector('#textArea');
const userQuizInputBtn=document.querySelector('#userQuizInputBtn');
let quizNum=0;
//let originalWords=[]; //원래 입력받은 텍스트를 변환한 배열
let words=[]; // 입력받은 텍스트에서 빈칸을 없앤 배열
let wordSet=[]; // 문제를 출제할 배열, words의 인텍스마다 배열로 생성
let shuffle=[]; // words의 값들을 섞어서 wordSet에 하나씩 push
let isCard=false;
...
function openAnswer(){
	...
	openAnswerBtn.classList.add('hide');
}

function removeCard(){
	try{
		for(i=0;i<wordSet[quizNum].length;i++){
...
function nextQuiz(){    
	if(quizNum>=words.length-1){
			...
			removeCard();
			quizNum+=1;
			openAnswerBtn.classList.remove('hide');
			cardSetting(quizNum);
	}
}
function cardSetting(quizNum){
	let quiz=wordSet[quizNum];
	nextBtn.classList.add('hide');
	for(let i=0;i<quiz.length;i++){
			let card=document.createElement('div');
			card.className='card';
			card.innerHTML=`<p>${quiz[i]}</p>`;
			card.style.background=randColorArray[rand()];
			problemcontainer.appendChild(card);
	}
	answerCard.innerHTML=`${words[quizNum]}`;
	answerCard.classList.add('hide');
	openAnswerBtn.classList.remove('hide');
	openAnswerBtn.addEventListener('click', openAnswer);
	nextBtn.addEventListener('click', nextQuiz);
	isCard=true;
}

userQuizInputBtn.addEventListener('click', ()=>{
	if(quizNum!==0 || isCard) removeCard();
	init();
	const originalText=userQuizInput.value;
	const inputText=userQuizInput.value.replace(/(\s*)/g, '');
	originalWords=originalText.split(',');
	words=inputText.split(',');
	for(i=0;i<words.length;i++){
			let imsi=Hangul.disassemble(words[i]);
			while(imsi.length > 0){
					let imsi2=imsi.splice(Math.floor(Math.random()*imsi.length),1)[0];
					shuffle.push(imsi2);
			}    
			wordSet.push(shuffle); 
			shuffle=[];
	}
	cardSetting(quizNum);
	alert(`${words.length}문제가 준비되었습니다. 퀴즈 화면을 클릭하세요`);
});
