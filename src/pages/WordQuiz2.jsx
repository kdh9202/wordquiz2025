...
export default function WordQuiz2() {
  // 기본 상태 관리
  ...
  // 현재 문제의 카드 생성
  const makeCurrentQuiz = (sentence) => {
    const cards = sentence
      .split('')
      .filter(char => char !== ' ') // 공백 문자 제외
      .map((char, index) => ({
        id: index,
        char,
  ...
  const fetchSavedQuizzes = async () => {
    try {
      const quizzesCollection = collection(db, 'wordquiz2')
    ...
    <>
      ...
      <div className="fixed inset-0 bg-white overflow-hidden pt-14">
        <div className="h-full flex flex-col p-8">
          {quizWords.length === 0 ? (
            <>
              <h1 className="text-2xl font-bold text-gray-700 mb-2 flex-none text-center">문장 완성 퀴즈</h1>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 mb-6 flex-none">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2 text-blue-800">
                      ...
                      <p className="text-sm">무작위로 섞인 글자들을 조합하여 문장을 완성하는 게임입니다.</p>
                    </div>
                    <div className="flex items-start gap-2 text-blue-800">
                      <svg className="w-5 h-5 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                      </svg>
                      <p className="text-sm">문제에 사용될 문장을 쉼표로 구분하여 입력하세요.</p>
                    </div>
                    <div className="flex items-start gap-2 text-blue-800">
                      <svg className="w-5 h-5 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                ...
                <div className="grid grid-cols-12 gap-4 mb-6 flex-none">
                  <div className="col-span-9">
                    <textarea
                      ...
                      placeholder="예시) 내 이름을 큰 소리로 두 번 외쳐, 연필 들고 마법 주문을 외워봐"
                    />
                  </div>
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-bold text-gray-700">문장 완성 퀴즈</h1>
                <div className="flex items-center gap-4">
                    ...
                    {showAnswer && (
                      <div className="flex gap-4">
                        {quizWords[currentQuizIndex].split('').map((char, index) => {
                          ...
                          const problemCard = currentCards.find(card => card.char === char)
                          return (
                            <div
                              key={index}
                              className={`w-16 h-16 ${problemCard?.color || cardColors[index % cardColors.length]} rounded-xl flex items-center justify-center text-white text-3xl font-bold shadow-lg`}
                            >
                              {char}
                            </div>
                          )
